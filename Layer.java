  ...
  public Layer colorReduction() {
    ...
    for (var h = 0; h < b.getHeight(); h++) {
      for (var w = 0; w < b.getWidth(); w++) {
        ...
        if (dictionary.containsKey(pixelInt)) {
          int value = dictionary.get(pixelInt);
          dictionary.put(pixelInt, ++value);
        } else {
          dictionary.put(pixelInt, 1);
    ...
    Set<Entry<Integer, Integer>> allColors = dictionary.entrySet();
    List<Entry<Integer, Integer>> allColorsList = new ArrayList<>(allColors);
    
    
    allColorsList.sort((a,c)->-a.getValue() + c.getValue());
    
    System.out.println(allColorsList.size());
